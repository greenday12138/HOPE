...
class zone():
    ...
    #  [c031 c029取底边中点，其余取中点]
    def get_zone(self,bbox):
        if self.current_cam == 31 or self.current_cam == 29:
    ...
    def is_ignore(self,zone_list,frame_list, cid):
        ...
        if zs == ze:
            ...
            if ze in [1,2]:
                return 2 #排除
            if zs!=0 and 0 in zone_list: # 出现掉头
                return 0 
            if fe-fs>1500: # 长时间停留在一个区域
                return 2
            if fs<2:
                if cid in [45]:
                    if ze in [3,4]:
                        return 1 # return 1 轨迹被存到sub_mot_list 
                    else:
                        return 2
            if fe > 1999:
                if cid in [41]:
            ...
            # 如果区域发生变化
            if cid in [1, 2, 3, 4, 5, 26, 29, 31, 32, 41, 42, 43, 44, 45, 46]:
                # 如果从支路进支路出，排除
                if zs == 1 and ze == 2:
                    return 2
                if zs == 2 and ze == 1:
                    return 2
            if cid in [41]: # 41 46是两个边界相机
                # 在41相机，车辆没有进出42相机
                if (zs in [1, 2]) and ze == 4: # 从支路进，且直接离开监控区域
                    return 2
                if zs == 4 and (ze in [1, 2]): # 从监控区域外进入摄像头，且从支路出
                    return 2
            if cid in [46]:
                # 在46相机，车辆没有进出45相机
                if (zs in [1, 2]) and ze == 3: # 从支路进，且直接离开监控区域
                    return 2
                if zs == 3 and (ze in [1, 2]): # 从监控区域外进入摄像头，且从支路出
                    return 2
            return 0
    ...
    def filter_bbox(self,mot_list,cid):
        new_mot_list = dict()
        yh = self.zones[cid].shape[0] # img的高
        for tracklet in mot_list:
            # if tracklet==1812:
            #     print(tracklet)
            tracklet_dict = mot_list[tracklet]
    ...
    def break_mot(self,mot_list,cid):
        ...
        for tracklet in mot_list:
            ...
            # time_break用于标识轨迹帧之间是否存在时间间隔过大的情况
            time_break = False
            # if tracklet == 714:
            #     print(tracklet)
            for f in frame_list:
